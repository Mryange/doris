    ...
    std::visit(
            Overload {[&](std::monostate& arg) -> void {
                      ...
                      [&](auto& agg_method) -> void {
                          ...
                          // POC: assume no null keys, bypass State/lazy_emplace_key null check,
                          // directly call hash_table->lazy_emplace with precomputed hash
                          auto& hash_table = *agg_method.hash_table;
                          for (uint32_t i = 0; i < num_rows; ++i) {
                              agg_method.template prefetch<false>(i);
                              void* it = nullptr;
                              hash_table.lazy_emplace(
                                      agg_method.keys[i], it, agg_method.hash_values[i],
...
void DistinctStreamingAggLocalState::_output_distinct_keys(Block* block) {
    ...
    for (size_t i = 0; i < key_size; ++i) {
        ...
        key_columns.back()->reserve(batch_size);
    }
    ...
    std::visit(
            Overload {[&](std::monostate& arg) -> void {
                      ...
                      [&](auto& agg_method) -> void {
                          ...
                          // POC: reuse pre-allocated buffer instead of per-call heap alloc
                          _output_keys_buffer.resize(batch_size * sizeof(KeyType));
                          auto* keys = reinterpret_cast<KeyType*>(_output_keys_buffer.data());
                          ...
                          // POC: directly insert raw data, bypassing virtual call
                          // (equivalent to MethodOneNumber::insert_keys_into_columns)
                          if (num_rows > 0) {
                              key_columns[0]->insert_many_raw_data(
                                      reinterpret_cast<const char*>(keys), num_rows);
                          }
